...
const getBookById = async () => {
  const { data, error } = await supabase.from("buku").select(`*, kategori(*), rak(*), judul, penulis, penerbit,tahun_terbit, jml_hal, deskripsi,cover`).eq("id", route.params.id);
  if (data) {
  }
  buku.value = data[0];
  loading.value = false;
